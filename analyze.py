...
playlistDistinctTracks = list(client.query("SELECT * FROM `umg-dev.pitching.track_echonest_data`").result(timeout=6000))
...
for track in playlistDistinctTracks:
    ...
    genre = track[16].encode('utf-8') if track[16] else None
    playlistDistinctTracksDictionary[isrc] ={uri_name : uri, energy_name: energy, liveness_name: liveness,
                                             ...
                                             track_artist_name: track_artist, genre_name: genre}
...
for playlistKey in playlistsDictionary:
    ...
    for isrc in playlistTracks:
        ...
        if trackData.get(mode_name):
            mode_name_data = 'mode_{0}'.format(trackData[mode_name])
            playlistMedian[mode_name_data] = playlistMedian[mode_name_data] + 1
    ...
    for isrc in playlistTracks:
        ...
        for key, value in playlistMedian.iteritems():
            track_value = get_track_value(trackData, key)
    ...
    for key, value in playlistWeights.iteritems():
        weight_final_value = 1/(value * 1.0) if value != 0 else float("inf")
...
pitchingTracks = list(client.query("SELECT * FROM `umg-dev.pitching.pitching_tracks_echonest_data`").result(timeout=6000))
...
for pitching_track in pitchingTracks:
    ...
    genre = pitching_track[16].encode('utf-8') if pitching_track[16] else None
    ...
    for playlist_to_pitch_key, playlist_to_pitch_value  in playlistsDictionary.iteritems():
        ...
        for key, value in playlist_median.iteritems():
            track_value = get_track_value(pitching_track_dictionary, key)
        ...
        if track_error > playlist_max_error:
            track_error_percentage = -1 * (100 - (100 * (playlist_max_error / track_error)))
